...
export const load = async ({ locals }) => {
   ...
   let engineList = async () => {
      ...
      let result = raw.map((value) => ({ ...value, model: value?.expand?.model_id?.name, customer: value?.expand?.customer_id?.name, isAvailable: engineHistory.find(v => v.history_number == 1 && v.engine_id == value.id) }));
