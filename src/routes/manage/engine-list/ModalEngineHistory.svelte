<script>
	...
	async function getEngineHistory(engineId) {
		let res = await fetch(`/api/engine-history?engine_id=${engineId}`);
		let json = await res.json();

		return json;
	}

	let data = getEngineHistory(engineId);

	onMount(async () => {
		state = 'LOADING';
		data = await getEngineHistory(engineId);
		state = 'SUCCESS';
	});
</script>

<Modal.Root let:id id="engine_history" position="right">
	<Modal.Header>
		<Modal.Title title="Engine History" />
		<Modal.Action>
	...
	<Modal.Body>
		...
		{:then result}
			...
			{#each result.engineHistory as d}
				<List.Item>
					<span>{d.date_performed}</span>
					<span class="text-right font-bold">{d.status}</span>
				</List.Item>
			{/each}
