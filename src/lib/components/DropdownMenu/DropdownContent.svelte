...
{#if $isOpen}
	<div id="dropdown_content" transition:fly={{ y: -5 }} bind:contentRect={contentEl} class="absolute rounded-md flex space-y-1 flex-col right-0 z-20 mt-2 p-1 bg-white shadow-md {size}">
		<slot />
	</div>
{/if}
