...
{#if isRefresh}
	<div transition:fade={{ duration: 200 }} class="absolute z-50 flex h-full w-full items-center justify-center gap-4 bg-slate-100/40 py-8 text-2xl font-bold backdrop-blur-sm backdrop-brightness-100">
		<svg role="status" class="-mt-px inline w-8 animate-spin fill-orange-600 text-slate-400 dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path
		...
		<span class="text-sm font-semibold text-slate-600">Refresh table . . .</span>
	</div>
{/if}

<div class="manage-r-header relative">
	{#if $isDelete}
		<div transition:fade={{ duration: 200 }} class="absolute bottom-0 left-0 z-10 mx-auto flex h-full w-full items-center justify-center space-x-3 bg-slate-100/80 px-7 py-7 shadow backdrop-blur-sm">
			<span>Delete {selectedRows.length} selected data?</span>
			<button class="bg-red-400 px-4 py-1 text-xs" on:click={() => modal$.show('confirm', selectedRows.map((v) => v.original.id).toString())}>Yes</button>
			<button class="bg-green-200 px-4 py-1 text-xs" on:click={handleReset}>Reset</button>
		</div>
	{/if}
	...
	<div class="manage-r-action relative flex-row items-end gap-3 xl:flex-col">
		...
		<div class="btn-group horizontal w-full items-center justify-center md:!hidden">
			<slot name="action" />
		</div>
	</div>
</div>
...
<div class="manage-r-content">
	...
	{#if lengthOfRow}
		<div class="flex w-full flex-col items-center justify-center gap-3 py-6">
			...
			<button class="bg-slate-200 px-3 py-2 text-center tracking-wide shadow hover:bg-slate-300" on:click={() => show('create')}> Create a new one </button>
		</div>
	{/if}
</div>
