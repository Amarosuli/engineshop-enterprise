...
{:else}
<div>
   <input in:fade type="text" bind:this={inputElement} bind:value on:blur={() => isEditing = false}>
</div>
{/if}
...
<style lang="postcss">
   div {
	...
	span {
		@apply flex justify-start items-center w-full h-full text-xs italic text-red-600;
	}
