...
export class Method {
   constructor(client, collectionName) {
      ...
      this.create = async (data) => {
         ...
         return { id: result.id, payload: result, message: 'success' }
      }
      ...
      this.delete = async (id) => {
         try {
            await client.collection(this.collectionName).delete(id);
         } catch (error) {
            return { status: error.status, message: 'failed' }
         }
      ...
      this.findByFilter = async (filter) => {
         return await client.collection(this.collectionName).getFirstListItem(filter);
      }
      this.getFullList = async (options) => {
         let result = await client.collection(this.collectionName).getFullList({
   ...
   MethodRegister() {
      ...
      for (const key in arguments) {
         if (Object.hasOwnProperty.call(arguments, key)) {
            ...
            this[methodName] = (args) => { return new method(args) }
         }
      }
   }
}
