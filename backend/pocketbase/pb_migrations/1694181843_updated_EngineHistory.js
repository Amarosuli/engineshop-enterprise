...
migrate(
	(db) => {
		...
		collection.options = {
			query:
				'SELECT engine_availability.engine_id, engine_availability.status, (ROW_NUMBER() OVER(partition by engine_availability.engine_id order by engine_availability.status DESC)) as HN, (ROW_NUMBER() OVER()) as id FROM engine_availability'
		};
